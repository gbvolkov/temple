...
def available_slug(connection: sqlite3.Connection, desired: str, content_id: str | None = None) -> str:
    ...
    suffix = 2
    while True:
        row = connection.execute("SELECT id FROM contents WHERE slug=?", (candidate,)).fetchone()
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    def public_content(content_type: str | None = None, limit: int = Query(50, ge=1, le=200)):
        query = "SELECT * FROM contents WHERE status='published'"
        params: list[Any] = []
        if content_type:
        ...
        params.append(limit)
        with connect(settings.database_path) as connection:
            return [row_to_content(row) for row in connection.execute(query, params).fetchall()]

    @app.get("/api/public/content/{slug}")
    def public_content_item(slug: str):
        with connect(settings.database_path) as connection:
            row = connection.execute("SELECT * FROM contents WHERE slug=? AND status='published'", (slug,)).fetchone()
        if not row:
            raise HTTPException(404, "Материал не найден")
        return row_to_content(row)
    ...
    def content_index(
        ...
        return {"items": [row_to_content(row) for row in rows], "total": total, "limit": limit, "offset": offset}
    ...
    def update_content(content_id: str, payload: ContentUpdate, user: dict = Depends(require("editor", mutation=True))):
        with transaction(settings.database_path) as connection:
            old = connection.execute("SELECT * FROM contents WHERE id=?", (content_id,)).fetchone()
            if not old:
                raise HTTPException(404, "Материал не найден")
            if old["version"] != payload.version:
                raise HTTPException(409, "Материал уже изменён другим редактором; обновите страницу")
            slug = available_slug(connection, payload.slug, content_id)
            version = old["version"] + 1
            connection.execute(
                """UPDATE contents SET slug=?,title=?,data_json=?,status='draft',
                   version=?,updated_at=? WHERE id=?""",
                (slug, payload.title.strip(), json.dumps(payload.data, ensure_ascii=False, sort_keys=True),
                 version, utc_now(), content_id),
            )
            snapshot(connection, content_id, user["id"])
            row = connection.execute("SELECT * FROM contents WHERE id=?", (content_id,)).fetchone()
        return row_to_content(row)
    ...
    def review_content(content_id: str, payload: VersionPayload, user: dict = Depends(require("editor", mutation=True))):
        with transaction(settings.database_path) as connection:
            ...
            if not row["migration_review_required"]:
                return row_to_content(row)
            connection.execute(
                "UPDATE contents SET migration_review_required=0,version=version+1,updated_at=? WHERE id=?",
                (utc_now(), content_id),
            )
            snapshot(connection, content_id, user["id"])
            row = connection.execute("SELECT * FROM contents WHERE id=?", (content_id,)).fetchone()
    ...
    def publish_content(content_id: str, payload: VersionPayload, user: dict = Depends(require("publisher", mutation=True))):
        with transaction(settings.database_path) as connection:
            ...
            if row["version"] != payload.version:
                raise HTTPException(409, "Материал уже изменён; обновите страницу")
            if row["migration_review_required"]:
            ...
            now = utc_now()
            connection.execute(
                "UPDATE contents SET status='published',published_at=?,version=?,updated_at=? WHERE id=?",
                (now, version, now, content_id),
            )
            snapshot(connection, content_id, user["id"])
            row = connection.execute("SELECT * FROM contents WHERE id=?", (content_id,)).fetchone()
        return row_to_content(row)
    ...
    def archive_content(content_id: str, payload: VersionPayload, user: dict = Depends(require("publisher", mutation=True))):
        with transaction(settings.database_path) as connection:
            ...
            if row["version"] != payload.version:
                raise HTTPException(409, "Материал уже изменён; обновите страницу")
            connection.execute(
                "UPDATE contents SET status='archived',version=version+1,updated_at=? WHERE id=?", (utc_now(), content_id),
            )
            snapshot(connection, content_id, user["id"])
            row = connection.execute("SELECT * FROM contents WHERE id=?", (content_id,)).fetchone()
        return row_to_content(row)
    ...
    def revisions(content_id: str, _: dict = Depends(require("viewer"))):
        with connect(settings.database_path) as connection:
            rows = connection.execute(
                "SELECT id,version,actor_id,created_at FROM revisions WHERE content_id=? ORDER BY version DESC", (content_id,),
            ).fetchall()
    ...
    def import_legacy(dry_run: bool = True, _: dict = Depends(require("admin", mutation=True))):
        ...
        if crawl_path and crawl_path.exists():
            ...
            full_result = execute_plan(settings.database_path, crawl_path, plan)
            leaflet_result = run_import(settings.database_path, settings.legacy_sections_path, media_manifest=manifest_path, leaflets_only=True)
            return {
                "dry_run": False,
        ...
        return run_import(settings.database_path, settings.legacy_sections_path, dry_run=dry_run, media_manifest=manifest_path)
